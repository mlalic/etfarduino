...
// $Revision: 1.3.4.10 $  $Date: 2009/12/31 18:45:13 $
#include <stdafx.h>
#include <math.h>
#include "AdaptorKit.h"

// Must specify the extern CComModule!
...
HRESULT CmwDevice::InitHwInfo(VARTYPE DataType,int DataBits,int maxchannels,INPUTTYPE inType,LPCWSTR adaptorname,LPCWSTR hwname)
{
    ...
    swprintf(tmpString,sizeof(tmpString)/sizeof(tmpString[0]), L"DllVer=%8.4f DriverVer=%8.4f",DllRevNum, DriverRevNum);
...
{
    CComBSTR Msg;
    Msg.LoadString((USHORT)code); // Must cast to USHORT for CComBSTR::LoadString to work
    HRESULT RetVal=S_OK;
    if (Msg.Length()==0)         
    {
...
wTimerID(0)
{
    ...
    wTimerRes = (std::min)((std::max)(tc.wPeriodMin, static_cast<UINT>(resolution)), tc.wPeriodMax);
...
bool LowResTimer::CallPeriod(double deltas) // returns true for success
{
    ...
    // if the delay is less than 1 millisecond, make it one millisecond.  This can
    // happen with acquisitions at rates higher than ~500,000 samples/second
    // Likely, this is the fix to geck 228416
    if(delta < 1)
        delta = 1;
...
bool HiResTimer::CallPeriod(double deltas) // returns true for success
{
    ...
    Shutdown=false;
    Period=deltas;
    Delay=static_cast<UINT>(floor(Period*1000));
	// Wait for 1ms less than needed
	if ( Delay > 1 )
		Delay -= 1;
	else
		Delay = 1;

	// Mark current time and set up callback to occur after 'Delay' milliseconds
	Time = FastTimer();
	wTimerID = timeSetEvent(
        Delay,                    // delay
        wTimerRes,                     // resolution (global variable)
...
{ 
	...
	// Test if wrong timer or shutting down
	...
	// Spin loop to wait until period is really over 
...
//_RPT1(_CRT_WARN,"Actual Period= %lf\n", thisptr->Time - thisptr->OldTime + thisptr->Error );

	// Save time before callback
    thisptr->OldTime = thisptr->Time;
    ...
    {
		// Calculate the amount of time that the callback took 
		thisptr->Error = (FastTimer() - thisptr->OldTime);

//_RPT1(_CRT_WARN,"Error= %lf\n",thisptr->Error);

		// Calculate a new delay that takes into account the time to complete the callback
                UINT newDelay = static_cast<UINT>((std::max)(0.0,floor((thisptr->Period-thisptr->Error)*1000)));
		// Wait for 1ms less than needed
		if ( newDelay > 1 )
			newDelay -= 1;
		else
			newDelay = 1;
		thisptr->Delay = newDelay;

//_RPT1(_CRT_WARN,"New Delay= %ld\n",thisptr->Delay);
